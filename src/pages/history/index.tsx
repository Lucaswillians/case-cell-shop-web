        ...
        <div className="mx-auto w-full max-w-3xl space-y-6">
          ...
          {history.length === 0 && !isHistoryLoading && !isHistoryError ? (
            <div className="rounded-2xl border border-border bg-card p-10 text-center text-sm text-muted-foreground">
              ...
              <p>Faça sua primeira compra e o histórico aparecerá aqui.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {history.map((item) => (
                ...
                >
                  ...
                  {/* ITEMS DO PEDIDO (CORRIGIDO AQUI 🔥) */}
                    ...
                    <div className="flex flex-wrap gap-3">
                      ...
                      {item.status !== "canceled" && (
                        <button
                          type="button"
                          disabled={
                            cancellingId === item.id || isCanceling
                          }
                          onClick={() => handleCancel(item.id)}
                          className="rounded-full border border-destructive bg-destructive/5 px-4 py-2 text-sm font-semibold text-destructive hover:bg-destructive/10 disabled:opacity-60"
                        >
                          {cancellingId === item.id && isCanceling
                            ? "Cancelando…"
                            : "Cancelar pedido"}
